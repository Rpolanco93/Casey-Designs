        ...
        <div className="new-product-page">
            ...
            <Form method={formMethod} className='product-form' action={actionUrl} encType='multipart/form-data'
                ...
                <div className="product-name-form">
                    ...
                    />
                    {actionResults != undefined && actionResults.error && actionResults.name &&
                        <p>{actionResults.name}</p>}
                </div>
                <div className="product-description-form">
                    <label htmlFor='description'>Description</label>
                    <p>Provide a detailed description of your product in 1000 characters or less. Highlight key features
                        to attract potential buyers.</p>
                    <textarea
                        name='description' id='description' defaultValue={product.description}
                        required maxLength='1000'
                    />
                    {actionResults != undefined && actionResults.error && actionResults.description &&
                        <p>{actionResults.description}</p>}
                </div>
                <div className="product-price">
                    ...
                    />
                    {actionResults != undefined && actionResults.error && actionResults.price &&
                        <p>{actionResults.price}</p>}
                </div>
                <div className="product-images-form">
                    ...
                    {product.id ?
                        <div>
                            <p>Current Photo:<img className='product-image-form' src={product.previewImage}
                                                  alt={product.name}/></p>
                            <div className='image-upload'>
                                <input
                                    ...
                                    name='image1'
                                />
                                {actionResults != undefined && actionResults.error && actionResults.previewImage &&
                                    <p>{actionResults.previewImage}</p>}
                            </div>
                        </div> :
                        <div className='image-upload'>
                            <input
